...
```python
# программа определения факта существования треугольника из 3 отрезков
a = float(input('Введи длину отрезка а: '))
b = float(input('Введи  длину отрезка b: '))
c = float(input('Введи  длину отрезка c: '))
